"""
cypher/governor.py — Spend Governor / Circuit Breaker  (v4.0)
...
- State persisten di SQLite (default ~/.cypher/governor.db) → tahan restart.
...
DEFAULT_DB = Path(os.environ.get("CYPHER_GOVERNOR_DB", "~/.cypher/governor.db")).expanduser()
KILLSWITCH_FILE = Path(os.environ.get("CYPHER_KILLSWITCH", "~/.cypher/HALT")).expanduser()
...
class GovernorLimits:
    ...
    def from_env(cls) -> "GovernorLimits":
        ...
        return cls(
            max_tx_usd=f("CYPHER_MAX_TX_USD"),
            daily_cap_usd=f("CYPHER_DAILY_CAP_USD"),
            session_cap_usd=f("CYPHER_SESSION_CAP_USD"),
            max_slippage_pct=f("CYPHER_MAX_SLIPPAGE_PCT") or 5.0,
            max_gas_multiple=f("CYPHER_MAX_GAS_MULTIPLE") or 4.0,
            max_tx_per_min=int(f("CYPHER_MAX_TX_PER_MIN") or 12),
            require_simulation=os.environ.get("CYPHER_REQUIRE_SIM", "1") != "0",
        )
...
class SpendGovernor:
    def __init__(self, limits: Optional[GovernorLimits] = None,
        ...
        self.session_id = session_id or os.environ.get("CYPHER_SESSION_ID", str(int(time.time())))
