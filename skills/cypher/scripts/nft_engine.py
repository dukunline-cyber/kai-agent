"""
cypher/nft_engine.py — Template NFT buy/sell lintas-marketplace.
    ...
    async def execute_buy(self, chain_id: int, items: list[dict],
        ...
        async with httpx.AsyncClient() as c:
            r = await c.post(
                ...
                json={"items": items, "taker": taker, "source": "cypher-agent"},
    ...
    async def execute_list(self, chain_id: int, listings: list[dict],
        ...
        async with httpx.AsyncClient() as c:
            r = await c.post(
                ...
                json={"params": listings, "maker": maker, "source": "cypher-agent"},
                headers={"x-api-key": self.api_key, "content-type": "application/json"},
            )
        r.raise_for_status()
        return r.json()
