...
async def reminders_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reminders = await asyncio.to_thread(list_reminders_db, chat_id)
    if not reminders:
        await update.message.reply_text("Ga ada reminder aktif.")
        return
    text = "\u23f0 *Reminder Aktif:*\n\n"
    for r in reminders:
        text += f"#{r['id']} — {r['text']}\nWaktu: {r['remind_at']}\n\n"
    text += "Hapus reminder: /delreminder [id]"
    await safe_send(update.message, text)


async def del_reminder_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Delete a reminder by ID."""
    if not is_allowed(update): return
    if not context.args:
        await update.message.reply_text("Format: /delreminder [id]")
        return
    try:
...
async def tts_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_allowed(update): return
    if not context.args:
        await update.message.reply_text("Format: /tts teks yang mau disuarakan")
        return
    if not EDGE_TTS_AVAILABLE:
        await update.message.reply_text("TTS tidak tersedia (edge-tts belum terinstall).")
        return
    text = " ".join(context.args)
    chat_id = update.effective_chat.id
    thread_id = (update.message.message_thread_id if update.message else (update.callback_query.message.message_thread_id if update.callback_query and update.callback_query.message else None))
    await context.bot.send_chat_action(chat_id=chat_id, message_thread_id=thread_id, action=ChatAction.RECORD_VOICE)
    save_dir = Path.home() / "ai-agent" / "downloads"
    save_dir.mkdir(parents=True, exist_ok=True)
    tts_path = str(save_dir / f"tts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp3")
    success = await text_to_speech(text, tts_path)
    if success:
        try:
            await context.bot.send_voice(chat_id=chat_id, message_thread_id=thread_id, voice=open(tts_path, "rb"), caption=text[:200])
        except Exception as e:
            await update.message.reply_text(f"Error kirim voice: {e}")
        finally:
            try: os.remove(tts_path)
            except: pass
    else:
        await update.message.reply_text("Gagal generate voice. Coba lagi nanti.")


async def db_stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reminder_count = conn.execute("SELECT COUNT(*) FROM reminders WHERE sent=0").fetchone()[0]
    file_count = conn.execute("SELECT COUNT(*) FROM file_log").fetchone()[0]
    db_size = os.path.getsize(str(DB_PATH)) if DB_PATH.exists() else 0
    conn.close()
    await safe_send(update.message,
        f"*Database Stats:*\n\n"
        f"Chat history: {history_count} messages\n"
        f"Memory notes: {memory_count}\n"
        f"Active reminders: {reminder_count}\n"
        f"Files logged: {file_count}\n"
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in chat_busy:
        ...
        await update.message.reply_text(
            f"🚫 Cancel diminta untuk: {desc} (udah {elapsed}s).\n"
            "Bakal stop di iterasi berikutnya — kalo lagi nungguin command shell, mungkin nunggu timeout dulu."
        )
    else:
        await update.message.reply_text("Ga ada task yang lagi jalan.")

# ========================
# MAIN
# ========================

async def soul_cmd(update, context):
    """Show SOUL.md content + credential status."""
    chat_id = update.effective_chat.id
    thread_id = (update.message.message_thread_id if update.message else (update.callback_query.message.message_thread_id if update.callback_query and update.callback_query.message else None))
    soul = load_soul()
    if not soul:
        await update.message.reply_text(
            "SOUL.md ga ada di ~/ai-agent/SOUL.md\n"
            "Kai jalan pake fallback prompt. Bikin file dulu biar bisa di-customize."
        )
        return
    creds = list_credentials()
    creds_status = f"Credentials available: {len(creds)} file ({', '.join(creds) if creds else 'kosong'})"
    preview = soul[:1500] + ("\n\n... (potong, total " + str(len(soul)) + " karakter)" if len(soul) > 1500 else "")
    await update.message.reply_text(
        f"SOUL.md aktif ({len(soul)} karakter).\n{creds_status}\n\n--- ISI ---\n{preview}"
    )

async def soul_reload_cmd(update, context):
    """Force reload SOUL.md (sebenernya udah auto-reload tiap call, ini cuma konfirmasi)."""
    soul = load_soul()
    if soul:
        await update.message.reply_text(f"SOUL.md di-reload. {len(soul)} karakter ke-load.")
    else:
        await update.message.reply_text("SOUL.md ga ada. Kai pake fallback prompt.")

async def creds_cmd(update, context):
    """List credential files yang available (cuma nama file, BUKAN isi)."""
    creds = list_credentials()
    if not creds:
        await update.message.reply_text(
            "Belum ada credential file.\n"
            "Bikin di ~/ai-agent/credentials/<platform>.env dengan format KEY=VALUE.\n"
            "Cek ~/ai-agent/credentials/README.txt buat format per platform."
        )
        return
    await update.message.reply_text(
        "Credential files yang Kai bisa pake:\n" +
        "\n".join(f"- {f}" for f in creds) +
        "\n\n(Isi ga di-display. Kai akses lewat source dalam tool_use.)"
    )
